...
const UserPaymentHistory = () => {
    ...
    useEffect(() => {
      if (user && user.email) {
        // Send the user's email to the backend to fetch payment history
        axios.get(`http://localhost:5000/user-payment-history/${user.email}`)
          .then(response => {
        ...
        <tbody>
          {paymentHistory.map(payment => (
            <tr key={payment._id} className="bg-white">
              ...
              <td className="border border-gray-300 px-4 py-2">{payment.selectedAmount}</td>
              <td className="border border-gray-300 px-4 py-2">{payment.selectedCategory}</td>
